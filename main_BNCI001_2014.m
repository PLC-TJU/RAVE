...
% 训练样本为：前24,48,72,96,120
% 测试样本为：后120,96,72,48,24
clear;clc;

fs=250;
resttime=fs*0+1:fs*2;
tasktime=fs*2+1:fs*6;

trainNum=[24,48,72,96,120];
testAcc=nan(8,length(trainNum),9);
for subject=1:9

    [~,~,data2,label2]=BNCI001_2014(subject);
...
%RAVE
for i=1:9
    load(['D:\CoreyLin\20230426\Result_BNCI001_2014\newResultSub_',num2str(i),'.mat'])
    for j=1:5
        acc(j,i)=max(RIGEL(j).testacc(end,:));
    end
end